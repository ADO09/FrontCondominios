...
export class CuotasIngresosConfComponent {
  ...
  onSelectChangeTipoPago(event:Event){
   ...
   getAllFilters(){
    ...
    if(this.selectValueTipoPago != '-1')valueSelectTipoPago = this.selectValueTipoPago


    this.ingresosService.getAllFilters(this.idFraccionamientoUsuer,valueSelectTipoPago).subscribe((data:any)=>{
  ...
  searchConfIngreso(search:any ,event:any){
    ...
    if (this.TipoPagoTemp.length != 0) {
      ...
      const matchingPropiedades = this.TipoPagoTemp.filter((u:any) => 
      ...
      const matchingPropiedades = this.ingresosTemp.filter((u:any) => 
      u.descripcion.toLowerCase().includes(search.toLowerCase()) || 
      u.periodo.toLowerCase().includes(search.toLowerCase()) );
      this.ingresosData = matchingPropiedades
    }
