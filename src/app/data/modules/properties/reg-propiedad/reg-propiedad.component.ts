...
export class RegPropiedadComponent {
  ...
  ngOnInit() {
    ...
    this.propietariosService
      .propietariosGetInquilinos(this.idFraccionamientoUsuer)
      .subscribe((r) => {
    ...
    this.propietariosService
      .propietariosGetPropietarios(this.idFraccionamientoUsuer)
      .subscribe((r) => {
        // this.propiedadesData = r.body;
        console.log(r);
        // console.log(this.propiedadesData);
  ...
  addPropiedad() {
    ...
    this.formData.set(
      'tipoPropiedadId',
      this.formPropiedades.get('tipoPropiedadId')?.value
    );
    this.formData.set(
      'claveCatastral',
      this.formPropiedades.get('claveCatastral')?.value
    );
    this.formData.set(
      'descripcion',
      ...
      if (r.icon == 'success') {
        setTimeout(() => {
          this.router.navigateByUrl(
            '/dashboard/' + INTERNAL_ROUTES.MODULO_PROPPRINC
          );
        }, 200);
      }
    });
